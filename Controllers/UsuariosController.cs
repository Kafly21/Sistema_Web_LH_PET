...
namespace LH_PET_WEB.Controllers
{
    ...
    public class UsuariosController : Controller
    {
        ...
        public async Task<IActionResult> Criar(UsuarioCreateViewModel model)
        {
            ...
            {
                Nome = model.Nome, // Novo campo
                Email = model.Email,
                Perfil = model.Perfil, Ativo = true,
                SenhaTemporaria = true, // Exige a troca no primeiro login
        ...
        public async Task<IActionResult> AlternarStatus(int id)
        {
            ...
            {
                TempData["Erro"] = "Você não pode desativar a sua própria conta."; return RedirectToAction(nameof(Index));
            }

            usuario.Ativo = !usuario.Ativo;
            _contexto.Usuarios.Update(usuario); await _contexto.SaveChangesAsync();

            TempData["Sucesso"] = $"Status de {usuario.Nome} alterado com sucesso."; return RedirectToAction(nameof(Index));
        }
    }
}

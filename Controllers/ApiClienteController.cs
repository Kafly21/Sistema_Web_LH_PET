...
namespace SP_03_UC08_LH_PET_WEB.Controllers
{
    ...
    public class ApiClienteController : ControllerBase
    {
        ...
        public async Task<IActionResult> AtualizarPerfil([FromBody] ApiPerfilUpdateDTO dto)
        {
            ...
            {
                var emailEmUso = await
                _contexto.Usuarios.AnyAsync(u => u.Email == dto.Email && u.Id != usuario.Id);
        ...
        {
            var cliente = await ObterClienteLogadoAsync(); if (cliente == null) return Unauthorized();
        ...
        public async Task<IActionResult> RemoverPet(int id)
        {
            ...
            var temAgendamentoPendente = await
            _contexto.Agendamentos.AnyAsync(a => a.PetId == id && a.Status == "Pendente");
        ...
        public async Task<IActionResult> ListarAgendamentos()
        {
            ...
            .ToListAsync(); return Ok(agendamentos);
 
        }

        [HttpPost("agendamentos")] public async Task<IActionResult>
        CriarAgendamento([FromBody] ApiAgendamentoDTO dto)
        {
            var cliente = await ObterClienteLogadoAsync(); if (cliente == null) return Unauthorized();

            var petPertenceAoCliente = await
            _contexto.Pets.AnyAsync(p => p.Id == dto.PetId && p.ClienteId == cliente.Id);
            if (!petPertenceAoCliente) return BadRequest(new { mensagem = "Pet inválido ou não pertence a você." });

            var horarioOcupado = await
            _contexto.Agendamentos.AnyAsync(a => a.DataHora == dto.DataHora && a.Status != "Cancelado");
            ...
            _contexto.Agendamentos.Add(novoAgendamento); await _contexto.SaveChangesAsync();
            
            return Ok(new { mensagem = "Serviço agendado com sucesso!" });
        }

        [HttpGet("horarios-disponiveis")] public async Task<IActionResult>
        ObterHorariosDisponiveis([FromQuery] string data, [FromQuery] string tipo)
        {
            ...
            var config = await
            _contexto.Configuracoes.FirstOrDefaultAsync(c => c.Id == 1); if (config == null) return BadRequest("Regras da clínica não configuradas.");
            int diaSemana = (int)dataEscolhida.DayOfWeek; if
            ...
            .ToListAsync();
            var horariosLivres = new List<string>(); TimeSpan atual = config.HoraAbertura; TimeSpan duracao =
            TimeSpan.FromMinutes(minutosServico);
            while (atual.Add(duracao) <= config.HoraFechamento)
            {
                TimeSpan fimAtual = atual.Add(duracao);
                bool temConflito = agendamentosDoDia.Any(a =>
                {
                    TimeSpan aInicio = a.DataHora.TimeOfDay; int aDuracaoMin = a.Tipo == "Consulta" ?
                    config.MinutosConsulta : (a.Tipo == "Banho" ? config.MinutosBanho : config.MinutosTosa);

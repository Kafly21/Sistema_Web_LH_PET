...
namespace LH_PET_WEB.Controllers
{
    ...
    public class AtendimentoController : Controller
    {
        ...
        {
            var agendamento = await
            _contexto.Agendamentos.FindAsync(id);
            if (agendamento != null)
            {
        ...
        public async Task<IActionResult> Prontuario(int agendamentoId)
        {
            ...
            {
            AgendamentoId = agendamento.Id, Agendamento = agendamento
            };
        ...
        SalvarProntuario(Atendimento model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var agendamento = await
                _contexto.Agendamentos.FindAsync(model.AgendamentoId);
                if (agendamento != null)
                {
        ...
        public async Task<IActionResult> Historico(string pesquisa)
        {
            ...
            {
                pesquisa = pesquisa.ToLower(); query = query.Where(a =>
                a.Agendamento!.Pet!.Nome.ToLower().Contains(pesquisa) || a.Agendamento.Pet.Cliente!.Nome.ToLower().Contains(pesquisa));
            }
        ...
        NovoAgendamento(Agendamento model)
        {
